...
def get_max_common_2(num_a, num_b):
    if num_a == num_b:
        return num_b
    if num_a % 2 == 0 and num_b % 2 == 0:
        num_a >>= 1
        num_b >>= 1
        return get_max_common_2(num_a, num_b) << 1
    elif num_a % 2 == 0 and num_a % 2 != 1:
        num_a >>= 1
        return get_max_common_2(num_a, num_b) << 1
    elif num_a % 2 != 0 and num_b % 2 == 0:
        num_b >>= 1
        return get_max_common_2(num_a, num_b)
    else:
        max_num = num_a if num_a > num_b else num_b
        min_num = num_b if num_a < num_b else num_b
        return get_max_common_2(max_num - min_num, min_num)
