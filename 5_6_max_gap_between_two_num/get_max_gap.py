...
def get_max_gap_3(num_list):
    ...
    count_list = [0] * (gap + 1)
    for i in range(len(num_list)):
        count_list[num_list[i] - min_value] += 1
...
def get_max_gap_4(num_list):
    ...
    for i in range(1, bucket_num):
        if buckets[i].min_value is None:
            continue
        if max_gap < buckets[i].min_value - left_max:
            max_gap = buckets[i].min_value - left_max
        left_max = buckets[i].min_value
    return max_gap


a = [2, 6, 3, 4, 5, 10, 9]
print(get_max_gap_4(a))
